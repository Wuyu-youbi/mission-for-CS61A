def shuffle(s):
    ...
    target_seq = []
    for i in range(mid):
        target_seq.append(s0[i])
        target_seq.append(s1[i])
    return target_seq
...
def balanced(m):
    ...
    torque_left = total_mass(end(left_arm)) * length(left_arm)
    torque_right = total_mass(end(right_arm)) * length(right_arm)

    if torque_left != torque_right:
        return False
    
    if not is_planet(end(left_arm)):
        if not balanced(end(left_arm)):
            return False
    
    if not is_planet(end(right_arm)):
