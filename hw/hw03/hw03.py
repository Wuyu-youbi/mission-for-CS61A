...
def interleaved_sum(num, f_odd, f_even):
    ...
    if num == 1:
        ...
        def help_function(k, use_odd):
            if k <= num:
                if use_odd:
                    return f_odd(k) + help_function(k + 1, False)
                else:
                    return f_even(k) + help_function(k + 1, True)
            else:
                return 0
        return help_function(1, True)
...
def count_dollars(sum_needed):
    ...
    def helper(remaining, bill):
        if remaining == 0:
            ...
            return helper(remaining - bill, bill) + helper(remaining, next_smaller_dollar(bill))
...
def count_dollars_upward(sum_needed):
    ...
    def helper(remaining, bill):
        if remaining == 0:
            ...
            return helper(remaining - bill, bill) + helper(remaining, next_larger_dollar(bill))
