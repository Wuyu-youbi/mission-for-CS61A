...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    lat_distance = get_lat(city_a) - get_lat(city_b)
    lon_distance = get_lon(city_a) - get_lon(city_b)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_a = distance(fake_city, city_a)
    distance_b = distance(fake_city, city_b)
    if distance_a > distance_b:
        return get_name(city_b)
    elif distance_a < distance_b:
        return get_name(city_a)
    else:
        print('all of them are close to the target place')
